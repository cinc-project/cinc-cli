...
- **Fully tested.** Every command ships with both unit tests against
  an in-process HTTP server and integration tests that run the real
  compiled binary against a live cinc-server-ng (and, by hand, a real
  CINC Server), so behavior is verified end-to-end before it lands.
...
make vet              # go vet
make fmt              # gofmt -w
make test-integration # the real binary against an in-process cinc-server-ng
...
- `integration/`: end-to-end tests of the binary against real server
  implementations, in their own Go module. See
